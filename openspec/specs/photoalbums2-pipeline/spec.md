## Purpose
Define the parallel `photoalbums2/` Dagster pipeline for page and photo work units, including prompt ownership, retry behavior, output modes, geocoding outputs, and review UI behavior while the legacy `photoalbums/` pipeline remains canonical.
