...
The requested restoration target is red-shift degradation caused by cyan pigment failure. The user specifically wants CTMs generated from stitched images using a Gemma 4 model and stored using Adobe Camera Raw-compatible metadata (`crs:ColorMatrix1`) in the `_Archive/` version of the XMP for the stitched page/image.

## Goals / Non-Goals

**Goals:**
- Add a Gemma 4-backed CTM estimation workflow for stitched photoalbum images.
- Persist CTMs in `_Archive/` XMP using `crs:ColorMatrix1` while preserving existing photoalbum metadata.
- Expose CTM generation and review through job-capable CLI and MCP entrypoints.
- Apply stored CTMs as deterministic linear transforms after stitching and before downstream render/export output handling, without modifying archival masters.

**Non-Goals:**
- Destructively rewriting `_Archive/*.tif` master scans.
- Adding stitch ingredient or homography metadata as part of this CTM change.
...
- **Camera Raw metadata in `_Archive/` XMP conflicts with existing photoalbum fields** → Extend namespace handling carefully and test coexistence with current XMP structures.
