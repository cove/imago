...
The system SHALL read the `mwg-rs:RegionList` from a view JPEG's XMP sidecar, convert each normalised centre-point region to a pixel rectangle, crop the corresponding area from the raw (pre-CTM) `_V.jpg`, and write the result as `_D{index:02d}-00_V.jpg` in the album's `_Photos/` sibling directory. CTM colour correction is applied to each crop separately by the subsequent `ctm-apply` pipeline step.
...
- **WHEN** the view sidecar has no `mwg-rs:RegionList` or the list is empty
...
The system SHALL write a fresh XMP sidecar for each crop containing `xmpMM:DocumentID`, `xmpMM:DerivedFrom` and `xmpMM:Pantry` (both referencing the source `_V.jpg`), `dc:source` (relative path to the `_V.jpg`), and `dc:description` resolved from the caption priority chain.

The caption priority chain for each crop is:
1. The region's `dc:description` from the `mwg-rs:RegionList` (set by the caption-association step)
2. The region's `caption_hint` stored in the `mwg-rs:RegionList` (the model's brief description for that region)
3. The view sidecar's page-level `dc:description` (inherited by all crops when no per-region caption exists)
4. Empty — no `dc:description` written on the crop sidecar

#### Scenario: Crop sidecar provenance links to view JPEG
...
- **WHEN** the view sidecar's `Iptc4xmpExt:LocationShown` contains two location entries
...
- **WHEN** the view sidecar has no GPS or city data
- **THEN** the crop sidecar contains no GPS or city fields

#### Scenario: Page date inherited by crops
- **WHEN** the view sidecar has `xmp:CreateDate` set to a year value
...
#### Scenario: --force overwrites existing crops
- **WHEN** `crop-regions --force` is run for a page with existing crops
- **THEN** all crops are regenerated and overwritten

### Requirement: --force removes orphaned crop files before re-cropping
The system SHALL, when `--force` is set, delete all existing `_D##-00_V.jpg` files (and their sidecars) in the `_Photos/` directory for the page before writing new crops. This prevents stale crops from a previous run with a different region count from persisting alongside the new crops.
...
- **THEN** `_D03-00_V.jpg` and its sidecar are deleted before the 2 new crops are written; only `_D01-00_V.jpg` and `_D02-00_V.jpg` exist after the run

#### Scenario: No deletion without --force
- **WHEN** `crop-regions` is run without `--force`
- **THEN** no existing crop files are deleted; only missing crops are written

### Requirement: Crop step completion is tracked in the view sidecar's pipeline state
The system SHALL write `pipeline.crop_regions` to the view JPEG's `imago:Detections` when all crops for a page complete successfully. If any crop fails, the state SHALL NOT be written. The step SHALL be skipped on re-run if the state record is present and `--force` is not set.

#### Scenario: Successful crop records pipeline state on view sidecar
- **WHEN** all crops for a page are written without error
- **THEN** the view sidecar's `imago:Detections` contains `{"pipeline": {"crop_regions": {"completed": "<iso-timestamp>"}}, ...}`

#### Scenario: Pipeline state skips crop step on re-run
- **WHEN** `pipeline.crop_regions.completed` is present in the view sidecar and `--force` is not set
...
- **WHEN** one region fails to crop (e.g. corrupt JPEG)
- **THEN** `pipeline.crop_regions` is not written; the next pipeline run retries all crops for that page
