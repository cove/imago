...
class TestPageDateSequence(unittest.TestCase):
    ...
    def test_sequence_album_page_dates_nudges_out_of_order_anchors(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Page 1 dated later than page 2 -> non-monotonic; must be nudged for ordering.
    ...
    def test_post_pass_sequences_each_album_once(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for page in ("01", "02"):
                _touch(archive / f"Family_1970-1974_B01_P{page}_S01.tif")
                _touch(pages / f"Family_1970-1974_B01_P{page}_V.jpg")
            _write_page_sidecar(pages / "Family_1970-1974_B01_P01_V.xmp", dc_date="1972")

            # Two page tuples for the same archive must sequence the album only once.
            all_pages = [
                (archive, pages, root / "Family_1970-1974_B01_Photos", []),
                (archive, pages, root / "Family_1970-1974_B01_Photos", []),
            ]
            counters = {"sequence-page-dates": {"run": 0, "skipped": 0, "failed": 0, "detail": []}}

            _run_sequence_page_dates_post_pass(all_pages, counters)

            self.assertEqual(counters["sequence-page-dates"]["run"], 1)
            self.assertEqual(counters["sequence-page-dates"]["failed"], 0)
            self.assertTrue(
                _read_text(pages / "Family_1970-1974_B01_P02_V.xmp", IMAGO_NS, "ViewerSortDate")
            )
