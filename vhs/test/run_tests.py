...
def test_step_6_badframe_sidecar_mapping():
    ...
    try:
        try:
            ...
            log.debug("load_badframe_repairs correctly raised ValueError: %s", exc)
    finally:
...
def _parse_bad_frames_from_tsv(frame_quality_tsv) -> set:
    ...
    for raw in frame_quality_tsv.read_text(encoding="utf-8", errors="ignore").splitlines():
        ...
        try:
            ...
            log.debug("failed to parse bad frame tsv row %r: %s", parts, exc)
            continue
        if is_bad:
            bad_exact.add(frame)
    return bad_exact
...
def _proxy_badframe_context() -> dict | None:
...
def _qtgmc_freeze_context() -> dict | None:
    proxy_path = _callahan_proxy_path()
